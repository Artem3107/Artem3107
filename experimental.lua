...
local Config = {
    ...
    AntiFling = false,
    BoostSpeed = 0.08,
    DashSpin = true   -- автоматический обход за спину при дэше
}
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aim Look (полностью автоматический DashSpin)
    if Config.Aim then
        local target = nil
        ...
        if target then
            ...
            local myVel = hrp.Velocity
            local horizSpeed = Vector3.new(myVel.X, 0, myVel.Z).Magnitude
            local dashThreshold = hum.WalkSpeed + 20

            -- Условия для автоматического обхода
            local shouldDashSpin = Config.DashSpin
                and horizSpeed > dashThreshold
                and dist > 5                                                   -- не в клинче
                and (not targetHum or (targetHum:GetState() ~= Enum.HumanoidStateType.Ragdoll
                                     and targetHum:GetState() ~= Enum.HumanoidStateType.GettingUp))
                and (math.abs(myVel.Y) <= horizSpeed)                         -- не прыжок/удар сверху

            if shouldDashSpin then
                -- Режим обхода за спину
                local behindDistance = 7
                local sideDistance = 6

                local toUs = (hrp.Position - targetRoot.Position) * Vector3.new(1,0,1)
                local enemyLook = targetRoot.CFrame.LookVector * Vector3.new(1,0,1)

                local side = toUs:Cross(enemyLook).Y > 0 and 1 or -1
                local perpendicular = Vector3.new(-toUs.Z, 0, toUs.X).Unit * side

                local behindPos = targetRoot.Position + (-enemyLook.Unit * behindDistance)
                targetPos = behindPos + perpendicular * sideDistance
            else
                -- Обычный режим с предсказанием
                local velocity = targetRoot.AssemblyLinearVelocity
                local ping = lp:GetNetworkPing() or 0.1
                local frameTime = 1 / math.max(fps, 1)
                local predictionTime = (ping * 0.5) + (frameTime * 2)
                targetPos = targetRoot.Position + velocity * predictionTime
            end
...
if Config.UnlockFPS then applyFPSUnlock() endlocal Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
local VirtualInputManager = game:GetService("VirtualInputManager")
local TeleportService = game:GetService("TeleportService")
